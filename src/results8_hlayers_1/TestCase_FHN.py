...
dt_num = 0.1#1                                 # numerical time step
...
for i in range(1,NTest+1):
  ...
  #b       = np.random.normal(0.9,0.5)
  b       = 0.8
  Iext    = np.random.uniform(0.5,0.9)#np.random.normal(0.5,0.2)
  #v0      = np.random.normal(1.5,5)
  v0      = 1.5
  w0      = v0
...
# Initial conditions
InitialValue_train = 0.6*np.ones((dataset_train['num_samples'], 2))
InitialValue_testg = 0.6*np.ones((dataset_testg['num_samples'], 2))
